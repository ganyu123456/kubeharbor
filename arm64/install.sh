...
#
# 前置条件:
#   - Linux arm64/aarch64（Ubuntu 20.04+ / Debian 11+ / 麒麟 / openEuler 等）
#   - root 权限
#   - 无需联网（完全离线）
...
[[ "$ARCH" != "aarch64" && "$ARCH" != "arm64" ]] && \
  error "此包仅支持 arm64/aarch64，当前架构: $ARCH"
...
echo ""
echo -e "${BOLD}╔══════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}║    Harbor 一键部署脚本  (arm64 离线版)       ║${NC}"
echo -e "${BOLD}╚══════════════════════════════════════════════╝${NC}"
echo ""
echo "  主机地址  : $HOST"
...
[ -f "$HARBOR_TMPL" ] || error "未找到 harbor.yml.tmpl，安装包可能不完整"
